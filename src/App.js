...
function App() {
  ...
  useEffect(() => {
    const getTasks = async () => {
      const tasksFromServer = await fetchTasks() //return an array
  ...
  const addTask = async (task) => {
    ...
    const data = await res.json() //this is the new task
    setTasks([...tasks, data])
  }
  ...
  const toggleReminder = async (id) => {
    ...
    const data = await res.json() //we get back the updated task
